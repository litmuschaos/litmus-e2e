...
var _ = Describe("BDD of openebs control plane chaos experiment", func() {
	...
	Context("Check for chaos engine verdict", func() {
		...
		It("Should check for the verdict of experiment", func() {
			...
			time.Sleep(10 * time.Second)
			chaosEngine, err := clientSet.ChaosEngines("openebs").Get(engineName, metav1.GetOptions{})
	...
	Context("Check for the result update", func() {
		...
		It("Should check for the result updation", func() {
			...
			err = utils.UpdateResultTable(clientSet, "OpenEBS control plane chaos test the control plane component", testVerdict, engineName, experimentName)
