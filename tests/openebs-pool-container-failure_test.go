...
var _ = Describe("BDD of openebs pool container failure experiment", func() {
		...
		It("Should check for creation of runner pod", func() {
			...
			if ChaosDurationFromEngine == "" {
				...
				if string(runner.Status.Phase) != "Succeeded" {
					...
					fmt.Println("Taking from experiment")
					fmt.Println("BRACE YOURSELF, EXPERIMENT BEGINS!")
					time.Sleep(time.Duration(ExperimentChaosDuration+200) * time.Second)
				...
				if string(runner.Status.Phase) != "Succeeded" {
					...
					fmt.Println("Taking from engine")
					fmt.Println("BRACE YOURSELF, EXPERIMENT BEGINS!")
					time.Sleep(time.Duration(EngineChaosDuration+200) * time.Second)
					runner, _ = client.CoreV1().Pods(chaosTypes.ChaosNamespace).Get(engineName+"-runner", metav1.GetOptions{})
					fmt.Printf("The Runner pod is in %v State EXPERIMENT ENDS!...\n", runner.Status.Phase)
					Expect(err).To(BeNil())
					Expect(string(runner.Status.Phase)).To(Equal("Succeeded"))
	...
	Context("Check Container Restart Count", func() {
		...
		It("Should check for the change in Container Restart Count after Chaos", func() {
			...
			podDetails.PodNamespace = chaosTypes.CspPodNs
			podDetails.PodLabel = chaosTypes.CspPodLabels
			containerRC, err := utils.CompareContainerRestartCount(containerDetailsBefore, podDetails, client)
