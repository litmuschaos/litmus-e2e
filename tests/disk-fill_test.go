...
var _ = Describe("BDD of disk-fill experiment", func() {
	...
	Context("Check for litmus components", func() {
		...
		It("Should check for creation of runner pod", func() {
			...
			rbacPath := "https://raw.githubusercontent.com/litmuschaos/chaos-charts/master/charts/kubernetes/disk-fill/rbac.yaml"
			...
			err = exec.Command("kubectl", "apply", "-f", "https://hub.litmuschaos.io/api/chaos?file=charts/kubernetes/disk-fill/experiment.yaml", "-n", chaosTypes.ChaosNamespace).Run()
			...
			err = exec.Command("wget", "-O", experimentName+"-ce.yaml", "https://raw.githubusercontent.com/litmuschaos/chaos-charts/master/charts/kubernetes/disk-fill/engine.yaml").Run()
			...
			err = exec.Command("sed", "-i",
				`s/namespace: default/namespace: litmus/g;
			         s/name: nginx-chaos/name: engine8/g;
