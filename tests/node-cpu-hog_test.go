...
var _ = Describe("BDD of node-cpu-hog experiment", func() {
	...
	Context("Check for litmus components", func() {
		...
		It("Should check for creation of runner pod", func() {
			...
			err = exec.Command("wget", "-O", "node-cpu-hog.yaml", "https://hub.litmuschaos.io/api/chaos/master?file=charts/generic/node-cpu-hog/experiment.yaml").Run()
			Expect(err).To(BeNil(), "fail get chaos experiment")
			err = exec.Command("sed", "-i", `s/litmuschaos\/go-runner:latest/`+chaosTypes.ExperimentRepoName+`\/`+chaosTypes.GOExperimentImage+`:`+chaosTypes.ExperimentImageTag+`/g`, "node-cpu-hog.yaml").Run()
	...
	Context("Check for the result update", func() {
		...
		It("Should check for the result updation", func() {
			...
			err = utils.UpdateResultTable(clientSet, "Exhaust CPU resources on the Kubernetes Node", testVerdict, engineName, experimentName)
