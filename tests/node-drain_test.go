...
var _ = Describe("BDD of node-drain experiment", func() {
	...
	Context("Check for litmus components", func() {
		...
		It("Should check for creation of runner pod", func() {
			...
			rbacPath := "https://raw.githubusercontent.com/litmuschaos/chaos-charts/master/charts/kubernetes/node-drain/rbac.yaml"
			...
			err = exec.Command("kubectl", "apply", "-f", "https://hub.litmuschaos.io/api/chaos?file=charts/kubernetes/node-drain/experiment.yaml", "-n", chaosTypes.ChaosNamespace).Run()
			Expect(err).To(BeNil(), "fail to create chaos experiment")
			if err != nil {
				fmt.Println(err)
			}
			...
			err = exec.Command("wget", "-O", experimentName+"-ce.yaml", "https://raw.githubusercontent.com/litmuschaos/chaos-charts/master/charts/kubernetes/node-drain/engine.yaml").Run()
			...
			err = exec.Command("sed", "-i",
				`s/namespace: default/namespace: litmus/g;
			         s/name: nginx-chaos/name: engine7/g;
