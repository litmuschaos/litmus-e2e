...
var _ = Describe("BDD of pod-delete experiment", func() {
	Context("Check for litmus components", func() {
		...
		It("Should check for creation of runner pod", func() {
			...
			rbacPath := "https://raw.githubusercontent.com/litmuschaos/chaos-charts/master/charts/kubernetes/pod-network-corruption/rbac.yaml"
			...
			err = exec.Command("kubectl", "apply", "-f", "https://hub.litmuschaos.io/api/chaos?file=charts/kubernetes/pod-network-corruption/experiment.yaml", "-n", chaosTypes.ChaosNamespace).Run()
			...
			err = exec.Command("wget", "-O", experimentName+"-ce.yaml", "https://raw.githubusercontent.com/litmuschaos/chaos-charts/master/charts/kubernetes/pod-network-corruption/engine.yaml").Run()
			...
			err = exec.Command("sed", "-i",
				`s/namespace: default/namespace: litmus/g;
			         s/name:  nginx-network-chaos/name: engine4/g;
