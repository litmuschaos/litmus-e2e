...
var (
	...
	err            error
	experimentName = "kubelet-service-kill"
	engineName     = "engine11"
)
...
var _ = Describe("BDD of kubelet-service-kill experiment", func() {
	...
	Context("Check for litmus components", func() {
		...
		It("Should check for creation of runner pod", func() {
			...
			err = exec.Command("wget", "-O", "kubelet-service-kill.yaml", "https://hub.litmuschaos.io/api/chaos/master?file=charts/generic/kubelet-service-kill/experiment.yaml").Run()
			Expect(err).To(BeNil(), "fail get chaos experiment")
			err = exec.Command("sed", "-i", `s/litmuschaos\/ansible-runner:latest/`+chaosTypes.ExperimentRepoName+`\/`+chaosTypes.ExperimentImage+`:`+chaosTypes.ExperimentImageTag+`/g`, "kubelet-service-kill.yaml").Run()
			...
			err = exec.Command("wget", "-O", experimentName+"-ce.yaml", "https://raw.githubusercontent.com/litmuschaos/chaos-charts/master/charts/generic/kubelet-service-kill/engine.yaml").Run()
	...
	Context("Check for the result update", func() {
		...
		It("Should check for the result updation", func() {
			...
			err = utils.UpdateResultTable(clientSet, "Kills the kubelet service on the application node.", testVerdict, engineName, experimentName)
