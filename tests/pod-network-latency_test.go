...
var _ = Describe("BDD of pod-network-latency experiment", func() {
	...
	Context("Check for pumba lib", func() {
		...
		It("Should check the pod-network-latency experiment when lib is set to pumba", func() {
			...
			By("[PreChaos]: Fetching all default ENVs")
			klog.Infof("[PreReq]: Getting the ENVs for the %v test", testsDetails.ExperimentName)
			environment.GetENV(&testsDetails, "pod-network-latency", "pod-network-lat-pumba")
		...
		// BDD for checking chaosengine Verdict when the latency check is set to true
		It("Should check for the verdict of experiment", func() {
	...
	Context("Check for the result update", func() {
		...
		It("Should check for the result updation", func() {
			...
			if testsDetails.UpdateWebsite == "true" {
				...
				testsDetails.EngineName = "pod-network-lat-pumba"
