...
func Exec(commandDetails *PodDetails, clients environment.ClientSets, command []string) (string, error) {
	...
	if err != nil {
		...
		if errorCode != true {
			if strings.Contains(err.Error(), "137") {
				klog.Info("Chaos process OOM killed as the provided value exceeds resource limits")
			} else {
				klog.Infof("[Prepare]: Unable to run command inside container, err : %v", err.Error())
			}
